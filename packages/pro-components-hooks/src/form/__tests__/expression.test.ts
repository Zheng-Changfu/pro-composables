  ...
  it('$row & $record', async () => {
    ...
    const Comp = defineComponent({
      setup() {
        ...
        onMounted(async () => {
          await nextTick()
          $rows.push({ ..._baseField.scope.value.$row.value })
          $records.push({ ..._baseField.scope.value.$record.value })
          _field.remove(1)
          await nextTick()
          $rows.push({ ..._baseField.scope.value.$row.value })
          $records.push({ ..._baseField.scope.value.$record.value })
  ...
  it('$total', async () => {
    ...
    const Comp = defineComponent({
      setup() {
        ...
        onMounted(async () => {
          await nextTick()
          $total.push(_baseField.scope.value.$total.value)
          _field.remove(1)
          await nextTick()
          $total.push(_baseField.scope.value.$total.value)
          _field.insert(1, { a: 33 })
  ...
  it('$index & $rowIndex', async () => {
    ...
    const Comp = defineComponent({
      setup() {
        ...
        onMounted(async () => {
          await nextTick()
          $indexs.push(_baseField.scope.value.$index.value)
          $rowIndexs.push(_baseField.scope.value.$rowIndex.value)
          _field.remove(1)
          await nextTick()
          $indexs.push(_baseField.scope.value.$index.value)
          $rowIndexs.push(_baseField.scope.value.$rowIndex.value)
